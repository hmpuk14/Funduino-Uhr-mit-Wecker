...
void checkAlarm() { // pruefen Uhrzeit gleich Weckzeit. Bei Alarm blinkt LED, bis Alarmzeit aus.
  if (currentHour == alarmHour && currentMinute == alarmMinute) {
    static unsigned long ledPreviousMillis = 0;
    const long ledInterval = 1000;  // Intervall für das Blinken der LED (1 Sekunde)
    
    unsigned long currentMillis = millis();
    if (currentMillis - ledPreviousMillis >= ledInterval) {
      ledPreviousMillis = currentMillis;
      digitalWrite(ledPin, !digitalRead(ledPin));  // LED umschalten
    }
  } else {
